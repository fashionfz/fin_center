package com.megaeyes.fin.install;

import static com.megaeyes.fin.common.Constants.UTF8;
...
public class Install {
	public static void dbXml(String fileName, InstallConfig config) throws Exception {
		String s = FileUtils.readFileToString(new File(fileName));
		s = StringUtils.replace(s, "DB_HOST", config.getDbHost());
		s = StringUtils.replace(s, "DB_PORT", config.getDbPort());
		s = StringUtils.replace(s, "DB_NAME", config.getDbName());
		s = StringUtils.replace(s, "DB_USER", config.getDbUser());
		s = StringUtils.replace(s, "DB_PASSWORD", config.getDbPassword());
	 ...
	 * @throws Exception
	 */
	public static void createTable(InstallConfig config, List<String> sqlList)
			throws Exception {
		Connection conn = getConn(config.getDbHost(), config.getDbPort(), config.getDbName(), config.getDbUser(), config.getDbPassword());
		Statement stat = conn.createStatement();
	 ...
	 * @throws Exception
	 */
	public static void updateConfig(InstallConfig config) throws Exception {
//		Connection conn = getConn(dbHost, dbPort, dbName, dbUser, dbPassword);
//		Statement stat = conn.createStatement();
