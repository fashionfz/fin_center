...
public class InstallServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String dbXmlFileName = "/WEB-INF/classes/c3p0.properties";
		...
		try {
			...
			if ("true".equals(isCreateTable)) {
				...
				Install.createTable(config,sqlList);
			}
			// 初始化数据
			if ("true".equals(isInitData)) {
				...
				Install.createTable(config ,initList);
			}
			// 更新配置
	        Install.updateConfig(config);
			// 处理数据库配置文件
			String dbXmlPath = getServletContext().getRealPath(dbXmlFileName);
			
			Install.dbXml(dbXmlPath, config);
			// 处理web.xml
